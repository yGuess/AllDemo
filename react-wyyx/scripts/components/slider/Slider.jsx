...
export default class Slider extends React.Component{
    ...
    componentDidMount(){
        // 是否显示spots
        this.timerInfo.isSpots = this.props.isSpots || true
        // 处理轮播图
    ...
    render(){
        return(
            <div className={styles.wrapSlider}>
                {
                    this.timerInfo.isSpots && <ol ref={s => this.slider = s}
                        className={styles.wrapSlideItems} 
                        style={this.state.translate}>
                        {this.items}
                    </ol>
                }
                <ol className={styles.wrapSlidePots}>
                    {
                        this.imgs.map((ele,index) => {
                            return <li key={index} className={this.timerInfo.spotPos == index?styles.active:''} data-pot={index}></li>
                        })
                    }
                </ol>
            </div>
